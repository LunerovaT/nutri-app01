...
export default function App() {
  ...
  // Odeslání formuláře → výpočet + uložení
  const handleFormSubmit = async (formData) => {
    ...
    try {
      ...
      console.error("Chyba při ukládání klienta:", err);
    }
    setScreen("plan");
  };
              ...
              <div key={client.id} style={s.clientCard}>
                <div style={s.clientInfo} onClick={() => handleOpenClient(client)}>
                  ...
                  <div style={s.clientKcal}>
                    Cílový příjem: <strong>{client.targetCalories} kcal</strong>
                  </div>
                </div>
                <div style={s.clientActions}>
                  <button onClick={() => handleOpenClient(client)} style={s.viewBtn}>
                    Zobrazit
                  </button>
                  <button onClick={() => handleDeleteClient(client.id)} style={s.deleteBtn}>
                    Smazat
                  </button>
                </div>
              </div>
...
function LoadingScreen({ text }) {
  return (
    <div style={{
      ...
      background: "#f0f4f1", fontFamily: "sans-serif",
      color: "#2d6a4f", fontSize: 16,
    }}>
      {text}
    </div>
  );
...
const s = {
  header: {
    ...
    boxShadow: "0 2px 8px rgba(0,0,0,0.15)", flexWrap: "wrap", gap: 8,
  },
  headerLeft:  { display: "flex", alignItems: "center", gap: 10 },
  headerLogo:  { fontSize: 24 },
  headerTitle: { color: "#fff", fontWeight: 700, fontSize: 18, fontFamily: "sans-serif" },
  ...
  signOutBtn: {
    ...
    border: "1px solid #c8e6d4", color: "#fff",
  ...
  newBtn: {
    padding: "10px 20px", background: "#2d6a4f",
  ...
  viewBtn: {
    padding: "7px 14px", background: "#2d6a4f",
