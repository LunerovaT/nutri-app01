    ...
    <div style={s.page}>
      ...
      <div style={s.header}>
        <div>
          ...
          <p style={s.subtitle}>
            Cílový příjem: <strong>{plan.targetKcal} kcal / {plan.targetKj} kJ denně</strong>
          </p>
        </div>
      ...
      <div style={s.summaryGrid}>
        ...
        <SummaryCard title="ℹ️ Jak číst jídelníček">
          <p style={s.legendText}>
            Z každého sloupce vyberte <strong>jednu potravinu</strong>. Při výběru více potravin 
            z jednoho sloupce upravte jejich množství úměrně — celkový příjem energie ze skupiny 
            musí zůstat stejný.
          </p>
        </SummaryCard>
      </div>
...
function MealCard({ meal }) {
  ...
  const hasCarbs  = meal.carbOptions.length  > 0 && meal.energyCarbs  > 0;
  const hasProt   = meal.protOptions.length  > 0 && meal.energyProt   > 0;
  const hasFat    = meal.fatOptions.length   > 0 && meal.energyFat    > 0;
  const hasFrVeg  = meal.frVegOptions.length > 0 && meal.energyFrVeg  > 0;

  const activeCols = [hasCarbs, hasProt, hasFat, hasFrVeg].filter(Boolean).length;
...
function Row({ label, value, highlight }) {
  return (
    <div style={s.summaryRow}>
      ...
      <span style={highlight ? { ...s.summaryValue, color: "#2d6a4f", fontWeight: 700 } : s.summaryValue}>{value}</span>
    </div>
  );
}

const s = {
  page:       { maxWidth: 1000, margin: "0 auto", padding: "20px 16px 60px", fontFamily: "sans-serif" },
  ...
  outlineBtn: { padding: "9px 18px", background: "transparent", border: "2px solid #2d6a4f", color: "#2d6a4f", borderRadius: 8, fontWeight: 700, cursor: "pointer", fontSize: 14 },
  solidBtn:   { padding: "9px 18px", background: "#2d6a4f", border: "2px solid #2d6a4f", color: "#fff", borderRadius: 8, fontWeight: 700, cursor: "pointer", fontSize: 14 },
  bottomBtns: { display: "flex", gap: 10, justifyContent: "center", marginTop: 32, flexWrap: "wrap" },

  summaryGrid:  { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14, marginBottom: 28 },
  summaryCard:  { background: "#fff", border: "1px solid #e0ece4", borderRadius: 10, padding: "14px 16px" },
  summaryTitle: { fontWeight: 700, fontSize: 13, color: "#2d6a4f", marginBottom: 10, paddingBottom: 6, borderBottom: "1px solid #e8f5ee" },
  summaryRow:   { display: "flex", justifyContent: "space-between", fontSize: 13, padding: "3px 0" },
  summaryLabel: { color: "#666" },
  summaryValue: { fontWeight: 600, color: "#333" },
  legendText:   { margin: 0, fontSize: 13, color: "#555", lineHeight: 1.6 },

  mealCard:   { background: "#fff", borderRadius: 12, border: "1px solid #e0ece4", marginBottom: 16, overflow: "hidden" },
  mealHeader: { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 16px", background: "#f0f8f3", flexWrap: "wrap", gap: 6 },
  ...
  colHeader:  { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5 },
  colEnergy:  { fontSize: 11, fontWeight: 600, opacity: 0.8 },
  foodRow:    { display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 12px", borderBottom: "1px solid #f5f5f5", fontSize: 13 },
